...
function buscarContacto(buscar){
  return function(x){
    console.log(buscar)
    return !x.Nombre.toLowerCase().indexOf(buscar.toLowerCase()) || !x.Apellido.toLowerCase().indexOf(buscar.toLowerCase());
  };
}
...
class App extends React.Component {
  ...
  eliminar = (contacto) => {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        opcion=false;
        swal({icon:"error",text:"Has cancelado, tu contacto está seguro!"});
      }
    });
  ...
  insertar= ()=>{
    var valorNuevo= {...this.state.contactos};
  ...
  render() {
    ...
    return (
      <>
        <Container className=" w-full sm:w-full sm:ml-5" style={{
        ...
        <h3 style={{marginLeft:"31.7%",fontFamily:"courier new"}}>LISTA DE CONTACTOS</h3>
        <div style={{}}>
           <Button color="primary"  size="md mb-2" onClick={()=>this.ensenarInsertar()} style={{
            background: "#141E30",
            background: "-webkit-linear-gradient(to right, #335e8f, #141E30)",  
            background: "linear-gradient(to right, #243B55, #141E30)"}}> <BsFillPersonPlusFill style={{marginRight:"8px",marginBottom:""}}/>Agregar</Button>
            <input type="text" className="rounded border-light" name="buscar" style={{marginLeft:"73.7%"}} placeholder="Buscar" onChange={this.cambioBuscar}></input>
            </div>
            <div style={{
            maxHeight: '250px',
            overflowY: 'auto'
          }}>
          <Table className="text-center w-full table table-bordered table-striped mb-0" style={{height:"10px"}}  striped bordered hover>
            <thead>
              <tr>
                <th scope="col">ID</th>
              ...
              this.state.datos.map((contacto) => (
                <tr key={contacto.id} style={{height:"10px"}}>
                  ...
                  <td scope="col">
                    ...
                    <Button color="secondary" onClick={()=> this.eliminar(contacto)}><BsTrash style={{marginRight:"2px",marginBottom:"4px"}}/>Eliminar</Button>
                  </td>
                </tr>
              ))
              :
              this.state.datos.filter(buscarContacto(this.state.buscar)).map((contacto) => (
                <tr key={contacto.id}>
                  <th scope="row">{contacto.id}</th>
                  <td scope="col">{contacto.Nombre}</td>
                  <td scope="col">{contacto.Apellido}</td>
                  ...
                  <td scope="col">
                    ...
                    <Button color="secondary" onClick={()=> this.eliminar(contacto)}> <BsTrash/>Eliminar</Button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <Modal isOpen={this.state.Actualizar}>
          <ModalHeader>
           <div style={{alignItems:"center",textAlign:"center"}}><h3>Editar Contacto</h3>
           <img src={Editr} style={{width:"55%",height:"2%"}}/></div>
          </ModalHeader>
        ...
        <Modal isOpen={this.state.Insertar}>
          <ModalHeader>
          <div style={{alignItems:"center",textAlign:"center"}}><h3>Insertar Contacto</h3>
